...
then
    ...
    ocamlfind ocamlopt -o $namenoext.out -package core,pads,pads.pads_ppx,re,unix,str,re.glob -linkpkg $tmp -thread -g
    rm $tmpfullnamenoext.cmx $tmpfullnamenoext.o $tmpfullnamenoext.cmi
    rm -f $fullnamenoext.cmx $fullnamenoext.o $fullnamenoext.cmi
else if (( $# == 2 )) && [ "$2" -eq "3" ];
then
    ocamlfind ocamlopt -o $namenoext.out -package pads.pads_ppx,re,core,unix,str,re.glob,pads -linkpkg $tmp -thread -g
    rm $tmpfullnamenoext.cmx $tmpfullnamenoext.o $tmpfullnamenoext.cmi
else if (( $# == 2 )) && [ "$2" -eq "4" ];
then
    $dir/desugar.sh $fullname
else
    ocamlfind ocamlopt -o $namenoext.out -package core,pads,pads.pads_ppx,re,unix,str,re.glob -linkpkg $fullname -thread -w -30
